# -*- coding: utf-8 -*-
'''
Graph Definition unit (Updated on 2021.8.19 by Ma Zicheng)

This unit originates from CS225, acts as fundamental definition for graph
used in Graph_Building_unit. Considering unique score algorithms, these
classes are updated, that is, some old functions are improved and some
...
class Graph:
    ...
    def All_connecting_vertexs(self, item, edges):
...
class VertexList:
    class __Vertex:
        def __init__(self, item, next=None, previous=None, score=1):
            self.item = item
            self.next = next
    ...
    def __init__(self, edges=[]):
        self.dummy = VertexList.__Vertex(None, None, None, None)
        self.numVertices = 0
    ...
    def append(self, item):
        ...
        newVertex = VertexList.__Vertex(item, self.dummy, lastVertex, 1)
    ...
    def __getscore__(self, index):
        cursor = self.dummy
        for i in range(index + 1):
            cursor = cursor.next
        return cursor.score

    def __setnewscore__(self, index, score):
        cursor = self.dummy
        for i in range(index + 1):
            cursor = cursor.next
        cursor.score = score
